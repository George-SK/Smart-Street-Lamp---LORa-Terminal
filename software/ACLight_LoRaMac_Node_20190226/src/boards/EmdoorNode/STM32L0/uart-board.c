...
//Modified by Steven. We use UART1. LPUART1--->USART1
...
void UartMcuInit( Uart_t *obj, uint8_t uartId, PinNames tx, PinNames rx )
{
    ...
    GpioInit( &obj->Tx, tx, PIN_ALTERNATE_FCT, PIN_PUSH_PULL, PIN_PULL_UP, GPIO_AF4_USART1 );	//Modified by Steven.
...
{
    HAL_UART_Transmit(&UartHandle,pData,Size,Timeout);
}
...
{
#if 0
    while( UartMcuPutChar( &Uart, ch ) != 0 );
    return c;
#else
...
if (ch == '\n') 
{
  status = HAL_UART_Transmit(&UartHandle, (void *)"\r", 1,30000);
  if (status != HAL_OK)
  {
    return EOF;
  }
}
status = HAL_UART_Transmit(&UartHandle, (uint8_t *)&ch, 1, 0xFFFF);
if (status != HAL_OK)
{
  return EOF;
}
