...
////Gpio_t AntSwitchTx;
////Gpio_t AntSwitchRx;
//Gpio_t AntSwitchRx;	//Modified by Steven.
//Gpio_t AntSwitchTx;
Gpio_t AntSwitch;
...
{
    return RF_PACONFIG_PASELECT_PABOOST;	//Modified by Steven in 2018/03/30.
...
{
//    GpioInit( &AntSwitchTx, RADIO_ANT_SWITCH_TX, PIN_OUTPUT, PIN_PUSH_PULL, PIN_PULL_UP, 0 );	//Modified by Steven.
//    GpioInit( &AntSwitchRx, RADIO_ANT_SWITCH_RX, PIN_OUTPUT, PIN_PUSH_PULL, PIN_PULL_UP, 1 );
	GpioInit( &AntSwitch, RADIO_ANT_SWITCH, PIN_OUTPUT, PIN_PUSH_PULL, PIN_PULL_UP, 1 );
}

void SX1276AntSwDeInit( void )
{
//    GpioInit( &AntSwitchTx, RADIO_ANT_SWITCH_TX, PIN_OUTPUT, PIN_OPEN_DRAIN, PIN_NO_PULL, 0 );	//Modified by Steven.
//    GpioInit( &AntSwitchRx, RADIO_ANT_SWITCH_RX, PIN_OUTPUT, PIN_OPEN_DRAIN, PIN_NO_PULL, 0 ); 
...
void SX1276SetAntSw( uint8_t opMode )
{
    switch( opMode )
    {
		case RFLR_OPMODE_TRANSMITTER:    // TX:1,RX:0    <--> Transmit
//        GpioWrite( &AntSwitchTx, 1 );	//Modified by Steven.
//        GpioWrite( &AntSwitchRx, 0 );
			GpioWrite( &AntSwitch, 0 );
			break;
    case RFLR_OPMODE_RECEIVER:
    case RFLR_OPMODE_RECEIVER_SINGLE:
    case RFLR_OPMODE_CAD:                // TX:0,RX:1    <--> RECEIVE
    default:
//        GpioWrite( &AntSwitchTx, 0 );	//Modified by Steven.
