...
{   
	...
	Rn820xInit();
	ProfileInit();
#if defined (GL5516_PHOTOREISTANCE_ENABLE)
	GL5516_init();
#endif
}
...
void BoardInitMcu( void )
{
    ...
    if( McuInitialized == false )
    {
        ...
        /* this code do something looks not in need */

        if( GetBoardPowerSource( ) == BATTERY_POWER )
        {
            //CalibrateSystemWakeupTime( );	//Modified by Steven in 2018/07/18.
        }
...
void SystemClockConfig( void )
{
	...
	HAL_SYSTICK_Config(HAL_RCC_GetHCLKFreq()/1000);	//Modified by Steven. Maybe:1000.
...
void SysTick_Handler( void )
{
    ...
    /*krhino_intrpt_enter();	//Commented by Steven in 2018/03/21.
