...
void RegionUS915InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
            // Channels
            // 125 kHz channels
            //for( uint8_t i = 0; i < US915_MAX_NB_CHANNELS - 8; i++ )	//Modified by Steven.
            ...
            // 500 kHz channels
            //for( uint8_t i = US915_MAX_NB_CHANNELS - 8; i < US915_MAX_NB_CHANNELS; i++ )	////Modified by Steven.
						for( uint8_t i = US915_MAX_NB_CHANNELS - 8; i < US915_MAX_NB_CHANNELS - 8 + 1; i++ )
            {
...
#else
    if( alternateDr->NbTrials < 8)
    {
        datarate = DR_4;
    }
    else if( alternateDr->NbTrials < 13)
    {
        datarate = DR_3;
    }
    else if( alternateDr->NbTrials < 18)
    {
        datarate = DR_2;
    }
    else if( alternateDr->NbTrials < 23)
    {
        datarate = DR_1;
    }
    else
...
bool RegionUS915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nbEnabledChannels > 0 )
    {
        ...
        *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
        ...
        //RegionCommonChanDisable( ChannelsMaskRemaining, *channel, US915_MAX_NB_CHANNELS - 8 );	//Commented by Steven.
