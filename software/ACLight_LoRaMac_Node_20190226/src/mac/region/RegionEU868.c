...
// Global attributes
/*!
 * The last datarate joining used.
 */
static uint8_t JoiningLastDatarate = DR_0;
...
PhyParam_t RegionEU868GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DEF_TX_DR:
        {
						//Modified by Steven in 2018/06/08.
            //phyParam.Value = EU868_DEFAULT_DATARATE;
						phyParam.Value = JoiningLastDatarate;
            break;
        }
        ...
        {
						phyParam.Value = 35;//20;//5;	//Modified by Steven in 2018/06/08. Org:48
            break;
        }
        default:
        {
...
bool RegionEU868Verify( VerifyParams_t* verify, PhyAttribute_t phyAttribute )
{
    switch( phyAttribute )
    {
        ...
        case PHY_DEF_TX_DR:
        {
						return RegionCommonValueInRange( verify->DatarateParams.Datarate, DR_0, DR_6 );	//Modified by Steven in 2018/06/08. Org: return RegionCommonValueInRange( verify->DatarateParams.Datarate, DR_0, DR_5 );
        }
        case PHY_RX_DR:
        {
...
bool RegionEU868TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    Radio.SetChannel( Channels[txConfig->Channel].Frequency );
		...
		//Uart1.PrintfLog("dr:%d, power:%d, bw:%d, freq:%d\r\n", phyDr, phyTxPower, bandwidth, Channels[txConfig->Channel].Frequency);	//Modified by Steven in 2018/06/08.
...
/*int8_t RegionEU868AlternateDr( AlternateDrParams_t* alternateDr )
{
    ...
    if( ( alternateDr->NbTrials % 5 ) == 0 )	//48
    {
        datarate = DR_0;
    }
    else if( ( alternateDr->NbTrials % 4 ) == 0 )	//32
    {
        datarate = DR_0;	//DR_1
    }
    else if( ( alternateDr->NbTrials % 3 ) == 0 )	//24
    {
        datarate = DR_2;	//DR_2
    }
    else if( ( alternateDr->NbTrials % 2 ) == 0 )	//16
    {
        datarate = DR_3;	//DR_3
    }
    else if( ( alternateDr->NbTrials % 1 ) == 0 )	//8
    {
        datarate = DR_4;	//DR_4
    }
    else
    {
        datarate = DR_5;	//Modified by Steven in 2018/05/11.
    }
    return datarate;
}*/
...
int8_t RegionEU868AlternateDr( AlternateDrParams_t* alternateDr )
{
    ...
    if( alternateDr->NbTrials < 6)
    {
        datarate = DR_6;
    }
    else if( alternateDr->NbTrials < 11)
    {
        datarate = DR_5;
    }
    else if( alternateDr->NbTrials < 16)
    {
        datarate = DR_4;
    }
    else if( alternateDr->NbTrials < 21)
    {
        datarate = DR_3;
    }
    else if( alternateDr->NbTrials < 26)
    {
        datarate = DR_2;
    }
		else if( alternateDr->NbTrials < 31)
    {
        datarate = DR_1;
    }
    else
