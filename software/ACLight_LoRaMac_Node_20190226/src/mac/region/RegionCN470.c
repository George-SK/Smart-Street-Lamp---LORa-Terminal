...
PhyParam_t RegionCN470GetPhyParam( GetPhyParams_t* getPhy )
{
    ...
    switch( getPhy->Attribute )
    {
        ...
        {
					phyParam.Value = 20;	//Modified by Steven in 2018/05/09. Org: phyParam.Value = 48;
            break;
        }
        default:
        {
...
void RegionCN470InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
            ...
            {
#if defined (CN470_NEW_CHANNEL_ENABLE)
                Channels[i].Frequency = 487900000 + i * 200000;
#else
								Channels[i].Frequency = 472300000 + i * 200000;
#endif
...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    Radio.SetTxConfig( MODEM_LORA, phyTxPower, 0, 0, phyDr, 1, 8, false, true, 0, 0, false, 3000 );
		...
		//Uart1.PrintfLog("TX Freq:%d DR:%d\r\n", Channels[txConfig->Channel].Frequency, txConfig->Datarate); //George@20180825:test code
...
/*int8_t RegionCN470AlternateDr( AlternateDrParams_t* alternateDr )
{
    ...
    if( ( alternateDr->NbTrials % 5 ) == 0 )
    {
        datarate = DR_0;
    }
    else if( ( alternateDr->NbTrials % 4 ) == 0 )
    {
        datarate = DR_0;
    }
    else if( ( alternateDr->NbTrials % 3 ) == 0 )
    {
        datarate = DR_2;
    }
    else if( ( alternateDr->NbTrials % 2 ) == 0 )
    {
        datarate = DR_3;
    }
    else if( ( alternateDr->NbTrials % 1 ) == 0 )
    {
        datarate = DR_4;
    }
    else
