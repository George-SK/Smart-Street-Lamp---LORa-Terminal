...
void RegionAU915InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
            // Channels
            // 125 kHz channels
            //for( uint8_t i = 0; i < AU915_MAX_NB_CHANNELS - 8; i++ )	//Modified by Steven.
            ...
            // 500 kHz channels
            //for( uint8_t i = AU915_MAX_NB_CHANNELS - 8; i < AU915_MAX_NB_CHANNELS; i++ )	//Modified by Steven.
						for( uint8_t i = AU915_MAX_NB_CHANNELS - 8; i < AU915_MAX_NB_CHANNELS - 8 + 1; i++ )
            {
